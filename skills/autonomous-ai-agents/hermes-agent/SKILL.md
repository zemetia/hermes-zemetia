...
/opt/data/memories/USER.md  Built-in user profile memory in this environment
```
...
When a user wants detailed durable memory for a domain (businesses, projects, research areas, etc.) and USER.md would become too dense, create a separate markdown file under `/opt/data/memories/` and leave USER.md with a concise pointer to it. Example: `/opt/data/memories/BUSINESS.md` for business/project context, with USER.md linking to that file instead of duplicating the full list. See `references/domain-memory-files.md` for the reusable structure and pitfalls.
...
### Browser tool in Docker/container cannot find Chrome
If the browser tool reports `Chrome not found` inside a Docker/container Hermes runtime, install the browser CLI/runtime in the same container filesystem the tool can see:
...
If Chrome installs under `/opt/data/home/.agent-browser` but the browser tool checks `/root/.agent-browser`, align the cache paths instead of concluding the browser is broken:

```bash
mkdir -p /root
ln -sfn /opt/data/home/.agent-browser /root/.agent-browser
```

For images that rebuild often, bake it into the Dockerfile and include system deps when needed:
...
Persist `/root/.agent-browser` (or the actual `$HOME/.agent-browser`) as a Docker volume to avoid re-downloading Chrome on every rebuild. After enabling the `browser` toolset, start a fresh Hermes session (`/reset` or restart) before testing.
